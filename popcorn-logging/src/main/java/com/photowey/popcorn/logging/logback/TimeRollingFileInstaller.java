 ...
 */
package com.photowey.popcorn.logging.logback;
...
import com.photowey.popcorn.logging.shared.RelaxedPropertyResolver;
...
public class TimeRollingFileInstaller {
    ...
    private boolean hasInstalled(LoggerContext loggerContext) {
        return null != loggerContext.getProperty(TimeRollingFileInstaller.class.getName());
    }

    private void markInstalled(LoggerContext loggerContext) {
    ...
    private RollingFileConf getRollingProperties(Environment environment) {
        RelaxedPropertyResolver propertyResolver = RelaxedPropertyResolver.ignoringUnresolvableNestedPlaceholders(environment, RollingFileConf.LOGGING_PREFIX);
